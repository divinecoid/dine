    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.bank-accounts.update', $bankAccount) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Tipe Pemilik <span class="text-red-500">*</span>
                    </label>
                    <div class="flex gap-4">
                        <label class="flex items-center">
                            <input type="radio" name="owner_type" value="brand" class="mr-2" onchange="toggleOwnerFields()" {{ $bankAccount->mdx_brand_id ? 'checked' : '' }}>
                            <span class="text-sm text-gray-700">Brand</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="owner_type" value="store" class="mr-2" onchange="toggleOwnerFields()" {{ $bankAccount->mdx_store_id ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="bank_name" name="bank_name" value="{{ old('bank_name', $bankAccount->bank_name) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 @error('bank_name') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <input type="text" id="bank_code" name="bank_code" value="{{ old('bank_code', $bankAccount->bank_code) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 @error('bank_code') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <input type="text" id="account_name" name="account_name" value="{{ old('account_name', $bankAccount->account_name) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 @error('account_name') border-red-500 @enderror">
                    @error('account_name')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="account_number" name="account_number" value="{{ old('account_number', $bankAccount->account_number) }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 @error('account_number') border-red-500 @enderror">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900">{{ old('notes', $bankAccount->notes) }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($bankAccount->is_verified)
                        <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-blue-100 text-blue-800">Terverifikasi</span>
                    @else
                        <span class="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-yellow-100 text-yellow-800">Belum Terverifikasi</span>
                        <p class="mt-2 text-sm text-gray-500">Gunakan tombol verifikasi di halaman daftar rekening untuk memverifikasi rekening ini.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-4 mt-6">
                <a href="{{ route('admin.bank-accounts.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Batal</a>
                <button type="submit" class="px-6 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
