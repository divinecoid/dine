...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Manage your menus</p>
        </div>
        <a href="{{ route('admin.menus.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form method="GET" action="{{ route('admin.menus.index') }}" class="flex flex-wrap gap-4">
            ...
            <select name="available" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900">
            ...
            <select name="category_id" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900">
            ...
            @if(request('search') || request('status') || request('available') || request('category_id'))
                <a href="{{ route('admin.menus.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Reset</a>
            @endif
        </form>
    </div>
                    ...
                    @forelse($menus as $menu)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        @if($menu->description)
                                            <div class="text-xs text-gray-500 mt-1 line-clamp-1">{{ Str::limit($menu->description, 50) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">Not Available</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($menu->is_active)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('admin.menus.edit', $menu) }}" class="text-gray-600 hover:text-gray-900">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <h3 class="mt-2 text-sm font-semibold text-gray-900">No menus found</h3>
            ...
            <div class="bg-white px-4 py-3 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        ...
                        @else
                            <a href="{{ $menus->previousPageUrl() }}" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Previous</a>
                        @endif
                        @foreach(range(1, min(5, $menus->lastPage())) as $page)
                            ...
                            @else
                                <a href="{{ $menus->url($page) }}" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">{{ $page }}</a>
                            @endif
                        @endforeach
                        @if($menus->hasMorePages())
                            <a href="{{ $menus->nextPageUrl() }}" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Next</a>
                        @else
